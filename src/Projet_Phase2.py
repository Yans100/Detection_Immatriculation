...
vid, delay = f.source_video() # Fonction qui demande à l'utilisateur de spécifier la source de la vidéo, webcam ou fichier
if vid == 0:
...
logo = cv2.imread("PhaseII_SIF1033/Fichiers/logo.png", cv2.IMREAD_UNCHANGED)
...
plaques_valides = ["M33 VJG","ABC123"]
...
while True:
    ...
    video = f.car_seg(video, 0.5, plaques_valides)
    ...
    wait = max(int((delay - elapsed) * 1000), 1)
    if cv2.waitKey(wait) & 0xFF == ord('q'): # Termine le programme si on appuie sur la touche "q"
        break
